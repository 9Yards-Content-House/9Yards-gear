    ...
    <section className="py-24 bg-card">
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {bundles.map((bundle) => (
            <Card key={bundle.id} className="bg-background border-border overflow-hidden group">
              ...
              <CardContent className="p-6">
                <h3 className="text-xl font-semibold text-foreground mb-2">{bundle.name}</h3>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Save {formatPrice(bundle.savings)} vs. individual
                    </div>
                  </div>
                  <Button asChild>
                    <Link href={`/contact?bundle=${bundle.id}`}>Book Bundle</Link>
                  </Button>
                </div>
              </CardContent>
            </Card>
